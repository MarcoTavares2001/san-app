...
<html>
    ...
    <body>
        <nav class="menu matematicas">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'matematicas', 'act3')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor matematicas">
           <h4>Actividad 3. Número.</h4>
           <span>Dibuja un balón a la niña para que tenga la misma cantidad que el niño.
    ...
    <script>
        fondo.src = 'public/img/dibujarPelota.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        enviar(<?=$_SESSION['id']?>, "matematicas", "act3", "<?=$_SESSION['nombre']?>");
    </script>
</html>
