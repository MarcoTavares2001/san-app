...
<html>
    ...
    <body>
        <nav class="menu exploracion">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'exploracion', 'act5')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor exploracion">
           <h4>Actividad 5. Hábitos de Higiene.</h4>
           <span>Observa las imágenes. Encierra las que puedes realizar por ti solo.
    ...
    <script>
        fondo.src = 'public/img/seleccionarHigiene.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        enviar(<?=$_SESSION['id']?>, "exploracion", "act5", "<?=$_SESSION['nombre']?>");
    </script>
</html>
