...
<html>
    ...
    <body>
        <nav class="menu exploracion">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'exploracion', 'act3')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor exploracion">
           <h4>Actividad 3. Las plantas ¿hay en mi jardín?.</h4>
           <span>Observa las plantas de tú jardín. Dibuja la que más te guste. Comenta ¿Porque elegiste esa planta? y ¿Cómo es?.
    ...
    <script>
        fondo.src = 'public/img/dibujarAnimales.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        enviar(<?=$_SESSION['id']?>, "exploracion", "act3", "<?=$_SESSION['nombre']?>");
    </script>
</html>
