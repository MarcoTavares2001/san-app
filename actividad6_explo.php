...
<html>
    ...
    <body>
        <nav class="menu exploracion">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'exploracion', 'act6')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor exploracion">
           <h4>Actividad 6. ¿Cómo me alimento?.</h4>
           <span>Colorea los alimentos saludables que te ayudan a crecer sano y fuerte.
    ...
    <script>
        fondo.src = 'public/img/seleccionarAlimento.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        enviar(<?=$_SESSION['id']?>, "exploracion", "act6", "<?=$_SESSION['nombre']?>");
    </script>
</html>
