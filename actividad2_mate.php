...
<html>
    ...
    <body>
        <nav class="menu matematicas">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'matematicas', 'act2')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor matematicas">
           <h4>Actividad 2. Número.</h4>
           <span>Dibuja la misma cantidad de dulces en el siguiente frasco.
    ...
    <script>
        fondo.src = 'public/img/dibujarCaramelos.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        enviar(<?=$_SESSION['id']?>, "matematicas", "act2", "<?=$_SESSION['nombre']?>");
    </script>
</html>
