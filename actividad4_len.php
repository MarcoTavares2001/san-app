...
<html>
    ...
    <body>
        <nav class="menu lenguaje">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'lenguaje', 'act4')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor lenguaje">
           <h4>Actividad 4. Trabalenguas "ERRE".</h4>
           <span>Repite el trabalenguas varias veces y colorea los objetos que se mencionan en el trabalenguas.
    ...
    <script>
        fondo.src = 'public/img/trabalenguasGuitarra.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        const cancion = cargarSonido("public/sound/trabalenguasGuitarra.mp3");
        ...
        enviar(<?=$_SESSION['id']?>, "lenguaje", "act4", "<?=$_SESSION['nombre']?>");
    </script>
</html>
