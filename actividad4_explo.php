...
<html>
    ...
    <body>
        <nav class="menu exploracion">
            ...
            <?php if($materia->verificar_act($_SESSION['id'], 'exploracion', 'act4')){ ?>
            <a id="download"><i class="fas fa-paper-plane"></i></a>
            <?php } ?>
        </nav>
        <div class="contenedor exploracion">
           <h4>Actividad 4. Aprendo a cuidar mi salud.</h4>
           <span>Observa e identifica, que artículo de higiene pertenece a cada parte del cuerpo y une con una línea.
                Comenta las características de estos artículos de higiene y menciona para que sirve cada uno de ellos.
    ...
    <script>
        fondo.src = 'public/img/lineasSalud.png';
        fondo.crossOrigin = "anonymous";
        fondo.onload = function(){
            context.drawImage(fondo, 0, 0, canvas.width, canvas.height);
        }
        enviar(<?=$_SESSION['id']?>, "exploracion", "act4", "<?=$_SESSION['nombre']?>");
    </script>
</html>
